...
- `## Idea Overview` — seed from the `## Goal` section of the existing `CLAUDE.md` (or
  ...
  [`references/new-project.md`](new-project.md) N1 Question 1).
- `## Core Features` — infer F1..Fn with priorities from the existing task list, docs, and graph
  ...
  style as [`references/new-project.md`](new-project.md) N1 Question 3).
- `## Out of Scope` — `[FILL IN]` placeholder.

### Step M3: Create `doc/app-flow.md`

Use the doc/app-flow.md template from [`references/existing-project.md`](existing-project.md) Step E7. If `graphify-out/graph.json` exists, populate the
...
1. Create `doc/design-brief.md` from the standard template ([`references/doc-templates.md`](doc-templates.md) File 4).
...
3. Get concrete tokens: if the codebase has a theme/token file, extract values from it; otherwise
   ask the vibe question ([`references/new-project.md`](new-project.md) N1 Question 4) and generate
   per **[`references/design-brief-generation.md`](design-brief-generation.md)**; approve.
...
Only if `[HAS_BACKEND]`: standard template ([`references/doc-templates.md`](doc-templates.md) File 5); populate the fence from the graph if present.
...
- Keep any non-checkbox lines (notes, sections like `## Completed (orchestrated)`) exactly where
  they are.
- Add the `## How to work this list` block (from the task-list format in [`references/doc-templates.md`](doc-templates.md)) after `## Goal`.
...
- Replace the `## Doc Navigation` and `## Rules` sections with the v2 versions ([`references/doc-templates.md`](doc-templates.md)
...
- **`## Key Architecture` needs one structural fix.** v1 put hand-written architecture prose inside
  ...
  [`references/fence-format.md`](fence-format.md)). While preserving the content verbatim, **split it**: graph-derived
